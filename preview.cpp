// ============================================================
// 文件：preview.cpp
// 职责：早期版本 SubReactor 的"历史档案馆"——整文件已被注释，不参与编译
//
// 【生活比喻：老照片相册】
//   这个文件就像一本老照片相册，记录了服务器在"过程式时代"的样子。
//   那时还没有 Session/Codec/Sender 的对象化拆分，所有逻辑——收包、解析、
//   路由、发送、关闭——都挤在 SubReactor 一个类里，像一个人同时当迎宾、
//   服务员、收银员和保洁。后来项目进入第三阶段对象化重构，这些职责被
//   拆分到 HttpSession、HttpCodec、ResponseSender 等专职类中，这段旧代码
//   就被整体注释保留下来，供学习对照"重构前后"的演进过程。
//
// 关键技术点（初学者重点理解，对照新版理解演进动机）：
//   1. EPOLLONESHOT + EPOLLET：旧版用"一次性触发+边缘触发"避免多线程惊群，
//      每次处理完必须手动 rearm 重新注册；新版改用协程挂起/恢复，不再手忙脚乱。
//   2. fd_close 的状态检查：防止"幽灵 fd"——fd 关闭后被复用给新连接，
//      旧线程误关新连接；新版用 Connection 的 id 自增做同一性校验。
//   3. pendingResponses 顺序发送：旧版用 map<seq,resp> 保证乱序完成时仍按序发送；
//      新版协程天然串行，无需自己维护序号。
//   4. eventfd 唤醒：旧版 Worker 线程完成任务后通过 eventfd 通知 Reactor 取结果；
//      新版通过协程调度器的 notifyExecuteComplete 实现更简洁的唤醒。
//
// 注意：以下代码全部被注释，仅供阅读对照，不会被编译器处理。
// ============================================================
