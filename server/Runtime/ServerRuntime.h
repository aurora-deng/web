...
// Router、Executor。所有组件在这里装配、启动、退出。
//
// 【生活比喻】
// ServerRuntime 是酒店"总经理"：手底下管着前台接待（acceptLoop）、一群楼层经理
// （ReactorGroup/SubReactor）、一位后厨经理（Executor）、一本菜谱（Router）。
...
//
// 关键技术点（初学者重点理解）：
// 1. 析构顺序很关键：reactorGroup_ 必须先于 executor_ 析构——确保 SubReactor 线程退出后
//    才停 Executor（避免 SubReactor 还在向 Executor 提交任务时 Executor 先被销毁）。
//    成员声明顺序 executor_ 在 reactorGroup_ 之后，符合"后声明先析构"的 C++ 规则。
...
// 5. 组件装配点：本类是整个服务器的"总装线"——构造期建 router_/executor_/wsManager_/
//    wsDispatcher_/wsFactory_/reactorGroup_ 六大组件，start() 时再 setupListener + createReactors
 ...
 *
 * 管理：监听套接字、epoll、SubReactor 组、Router 和共享 Executor。
 * 协议 Codec 由各 Session 自持，不再在 Runtime 层持有 HttpCodec；出站发送由 OutboundTask + TransportWriter 体系承担。
 *
 * 【通俗解释】
 * 本类是整个服务器的"总装线"——所有组件在这里装配、启动、退出：
 * 构造期建 router_/executor_/wsManager_/wsDispatcher_/wsFactory_/reactorGroup_ 六大组件，
...
private:
    ...
    Executor executor_;                              // 共享业务执行器（线程池封装）
    int port_ = 8080;                                // 监听端口
