...
//   以及超时/关闭时的生命周期钩子。具体怎么招待由 HttpSession / WebSocketSession 实现。
//
// 关键技术点（初学者重点理解）：
...
//      子类可选 override 填细节（如 WS 心跳、manager 注销），默认实现保持兼容。
//   3. 【多态派发】子类自持具体的编解码器（HttpCodec/WebSocketCodec）。Reactor 持有
//      基类指针，无需 if/else 分支判断协议类型。出站发送走协议无关的
//      OutboundTask + TransportWriter + writerLoop 体系。
//   4. 【双协议对称】HttpSession 与 WebSocketSession 都继承本类，对上层呈现
//      同一接口；同一套 Reactor / Connection 代码可透明承载两种协议。
 ...
 *
 * 【通话接线员岗位牌 通俗解释】
 *   Session 是 Connection 上的协议生命周期所有者。HTTP/1.1 与 WebSocket 共享
 *   协程调度约定；Codec 由子类自持（HttpSession 持有 HttpCodec，WebSocketSession
 *   持有 WebSocketCodec）。SubReactor 持有 Session 基类指针，无需关心具体协议类型。
 ...
 *   填细节（如 WebSocket 还要发 Close 帧、向 manager 注销自己）。默认实现保持兼容——
