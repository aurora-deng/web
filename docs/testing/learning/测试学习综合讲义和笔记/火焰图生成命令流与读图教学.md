...
> **目标**：会生成、会读图、能把图和第四阶段架构（SubReactor / 协程 / Executor / OutboundTask + OutboundQueue + TransportWriter + writerLoop）对上号。
...
webserver
  ...
  → HttpSession::run
  → SubReactor::enqueueOutbound（经 OutboundQueue 入队） 或  Executor::submit
（出站冲刷协程：SubReactor::writerLoop → TransportWriter::flush → writev）
```
...
| 结论 | 说明 |
|------|------|
| 架构可见 | SubReactor + 协程 resume + HttpSession + OutboundTask/OutboundQueue/TransportWriter/writerLoop + Executor 栈完整 |
...
4. 比较 `TransportWriter::flush`（writerLoop 冲刷）塔 vs `Executor::submit`/futex 塔——谁更宽？  
